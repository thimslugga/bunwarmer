...
#[cfg(target_os = "linux")]
use nix::unistd::{close, lseek, read, Whence};
...
fn read_chunk(fd: i32, buffer: &mut [u8], offset: u64) -> std::io::Result<usize> {
    lseek(fd, offset as i64, Whence::SeekSet)?;
...
async fn prewarm_device(
    ...
    for i in 0..num_workers {
        ...
        tokio::spawn(async move {
            ...
            #[cfg(target_os = "linux")]
            let _ = close(fd); // Ignore potential close errors

            let _ = tx.send(()).await; // Ignore potential send errors
        });
    }

    // Wait for all of the worker tasks to complete.
    for _ in 0..num_workers {
        let _ = rx.recv().await; // Ignore potential receive errors
    }
    ...
    Ok((device_path, total_bytes, duration))
}
